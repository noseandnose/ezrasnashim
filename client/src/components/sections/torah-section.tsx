      ...
      <div className="grid grid-cols-2 gap-3 h-full">
        {torahItems.map(({ id, icon: Icon, title, subtitle, color }) => (
          <div key={id} className="flex flex-col">
            {/* Show sponsorship bar above Daily Halocha and Daily Mussar */}
            {(id === 'halacha' || id === 'mussar') && (
              <SponsorshipBar contentType={id as 'halacha' | 'mussar'} />
            )}
            <div
              className="content-card rounded-2xl p-4 cursor-pointer hover:scale-105 transition-transform flex-1"
