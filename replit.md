...
Button interaction fix (Nov 2025): Fixed critical button freeze bug when app is minimized/backgrounded in PWA/app environments. Root cause: fullscreen modal system uses `requestAnimationFrame` for scroll-lock cleanup, which doesn't execute when app is backgrounded, leaving `pointer-events: none` stuck on scroll container. Solution: Created environment detection utility (`client/src/utils/environment.ts`) that identifies web view containers via user agent heuristics and visualViewport API absence. Modified fullscreen modal cleanup to use immediate synchronous restoration in web views while preserving double-requestAnimationFrame for regular browsers (gesture protection). Ensures pointer-events are properly restored even when JavaScript event loop is suspended during backgrounding.
Push notification fix (Nov 2025): Disabled push notification subscription in web view environments (Flutter, etc.) where IndexedDB/Cache Storage is often unavailable or restricted, causing "Registration failed - storage error" exceptions. Auto-notification prompt now detects web views and skips service worker registration, while permission manager validates storage availability before attempting subscription.
