...
Button interaction fix (Dec 2025): Fixed critical button freeze bug in mobile environments (FlutterFlow app, mobile browsers) WITHOUT losing user state. Root cause: Mobile webviews can swallow touchend/click events entirely after background/resume cycles, making all buttons unresponsive (CSS :active still works since pointerdown fires, but the click event never reaches JavaScript). Previous solution was reload-on-resume which fixed buttons but caused users to lose audio playback position and prayer scroll position. New solution: Click synthesis (click-synthesis.ts) that synthesizes click events when native clicks aren't firing after resume. Stays active until multiple consecutive native clicks confirm interactivity is restored. Preserves audio, scroll position, and app state. Fallback reload mode available via localStorage.setItem("forceReloadOnResume", "true"). Debug mode: localStorage.setItem("debugClickSynthesis", "true"). Also includes DOM event bridge (dom-event-bridge.ts) as safety net with data-action attributes. Modal click-through bug fixed by deferring onClose calls with requestAnimationFrame.
