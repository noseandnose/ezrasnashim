...
### Recent Maintenance (October 5-7, 2025)
- **Service Worker MIME Type Fix v4.7 (October 7)**: Enhanced MIME type protection with triple-layer defense system. Inline emergency recovery script in index.html runs BEFORE any code loads, automatically catches corrupted cache errors, clears all service workers/caches, and force-reloads the page - completely automatic for users with no manual intervention needed. Service worker v4.7 adds strict validation to never cache HTML as JavaScript, validates cached responses before serving, and auto-deletes corrupted entries. Users experience 1-2 second auto-recovery instead of blank screens.
- **Service Worker MIME Type Fix v4.6 (October 6)**: Refined cache cleanup from clearing all caches to targeted old-version cleanup. Auto-update system pushes new versions within 30 seconds without hard refresh.
- **Mobile Performance Optimizations (October 6)**: Enhanced resource loading with preconnect for Hebcal/Sefaria APIs and dns-prefetch for Maps/Stripe/Storage. Improved viewport settings with zoom support (5x), viewport-fit for notched devices, format-detection, and HandheldFriendly meta tags. Enhanced PWA with black-translucent status bar for iOS.
