...
- **Optional User Authentication**: Supabase Auth integration (Google OAuth, email/password) - 100% optional, app works fully without login. Authentication is handled entirely client-side with Supabase JS SDK. When logged in: personalized greeting with first name, profile page (/profile) showing daily progress stats, "My Profile" and "Sign Out" menu items. When logged out: "Create Profile" option in hamburger menu navigates to /login page. Auth routes: /login (sign in/sign up form), /auth/callback (OAuth redirect handler). User profile fields: firstName, lastName, hebrewName (RTL, optional), birthday (optional with future date validation), email, profileImageUrl - all stored in Supabase user_metadata. Profile editing available via pencil icon on profile page.
