...
Button interaction fix (Nov 2025): Fixed critical button freeze bug in mobile webview environments (FlutterFlow, etc.). Root cause: Mobile webviews can swallow touchend/click events entirely after certain background/resume cycles, making all buttons unresponsive (CSS :active still works since pointerdown fires, but the release event never reaches JavaScript). Solution: Implemented reload-on-resume system (flutterflow-reload.ts) that automatically detects webview environments using User Agent heuristics and global API detection, then reloads the page when the app regains visibility after being backgrounded. Uses visibilitychange and pageshow events with debouncing. System includes manual override via localStorage.setItem("forceReloadOnResume", "true") and optional debug logging via localStorage.setItem("debugReloadOnResume", "true"). Does NOT activate on regular mobile browsers (Safari, Chrome) to avoid UX regression. Also fixed modal click-through bug where clicking X button would trigger buttons beneath the modal - all onClose calls now deferred with requestAnimationFrame to prevent event retargeting during DOM unmounting. Additionally removed pointer-events manipulation from modal scroll locking (overflow-only) to eliminate CSS-based click blocking. Console logging gated behind debug flags (debugReloadOnResume, debugDOMBridge) for production cleanliness. This ensures reliable button functionality in webview environments without affecting normal browser users.
