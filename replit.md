...
Button interaction fix (Nov 2025): Fixed critical button freeze bug in FlutterFlow web view environment. Root cause: Fullscreen modal refactor introduced `pointer-events: none` on scroll container to prevent background interaction. When FlutterFlow suspends the page during backgrounding, React lifecycle hooks (useLayoutEffect) never run, leaving pointer-events stuck. This blocks React's event delegation while CSS `:active` still works (explaining why visual feedback appears but clicks don't fire). Solution: Webview-specific scroll locking strategy that avoids pointer-events entirely. In webviews (detected via environment.ts), modals now use overflow-only locking (the original approach that worked reliably). In regular browsers, enhanced locking with both overflow and pointer-events is maintained for better background scroll prevention. This eliminates the resume/cleanup complexity while preserving functionality across all environments.
