...
Button interaction fix (Nov 2025): Fixed critical button freeze bug specific to FlutterFlow web view environment. Root cause: FlutterFlow web views suspend ALL visibility events (visibilitychange, focus, pageshow) AND requestAnimationFrame while app is backgrounded, preventing standard resume handlers from executing. Meanwhile, modal locking logic applies `pointer-events: none` to `.content-area` and bottom navigation, which persists after app resume since cleanup code never runs. Solution: (1) Implemented FlutterFlow-specific polling mechanism (500ms interval) that detects visibility state changes without relying on events. (2) On resume detection, uses setTimeout (not rAF) to immediately force `pointer-events: auto` on all affected elements (.content-area, .mobile-app, bottom-nav, body, interactive elements). (3) Added diagnostic logging to FlutterFlow console for verification. (4) Standard event-based restoration maintained for PWA/website. (5) Updated fullscreen modal cleanup to use immediate synchronous restoration when `document.visibilityState !== 'visible'` or in web views. This ensures pointer-events are properly restored even when JavaScript event loop is suspended during backgrounding in FlutterFlow.
