...
- **Service Worker MIME Type Fix v4.9 (October 7)**: Final comprehensive fix for MIME type corruption. Service worker now never intercepts or caches sw.js (prevents old SWs from blocking updates) or index.html (ensures recovery scripts always load fresh). Uses updateViaCache: 'none' for automatic updates. Triple-layer defense: inline emergency recovery script in index.html, strict MIME validation before caching, and auto-deletion of corrupted entries. Automatic recovery within 1-2 seconds for all users.
