...
Button interaction fix (Nov 2025): Fixed critical button freeze bug in FlutterFlow mobile app. Root cause identified: FlutterFlow webview removes ALL document-level event listeners AFTER the first tap following resume from background, affecting both React's event delegation and the DOM Event Bridge. Solution: Nuclear option implemented - intercept document.removeEventListener calls to make our click/touchend handlers indestructible. When FlutterFlow (or any code) tries to remove click/touchend listeners from document, we immediately re-attach our handlers on the next microtask. If something tries to remove OUR specific handlers, the removal is blocked and we re-attach. Combined with visibilitychange listener for redundancy. The attachListeners() function is idempotent. DOM bridge provides resilience via capture-phase document listeners with data-action attributes, bypassing React's event delegation entirely. Also fixed modal click-through bug where clicking X button would trigger buttons beneath the modal - all onClose calls now deferred with requestAnimationFrame to prevent event retargeting during DOM unmounting. Additionally removed pointer-events manipulation from modal scroll locking (overflow-only) to eliminate CSS-based click blocking. Console logging gated behind debug flags (debugReloadOnResume, debugDOMBridge) for production cleanliness. Reload-on-resume workaround (flutterflow-reload.ts) disabled since root cause is fixed - this preserves audio playback when users minimize mid-listen.
