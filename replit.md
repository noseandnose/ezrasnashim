...
Button interaction fix (Nov 2025): Fixed critical button freeze bug in FlutterFlow mobile app. Root cause identified: FlutterFlow webview removes ALL document-level event listeners (including the DOM Event Bridge's click/touchend handlers) during background/resume cycles. Solution: Implemented automatic listener re-attachment on visibilitychange event - when app resumes from background, DOM bridge immediately re-attaches its capture-phase click/touchend listeners. The attachListeners() function is idempotent (can be called multiple times safely). DOM bridge provides resilience via capture-phase document listeners with data-action attributes, bypassing React's event delegation when it fails. Also fixed modal click-through bug where clicking X button would trigger buttons beneath the modal - all onClose calls now deferred with requestAnimationFrame to prevent event retargeting during DOM unmounting. Additionally removed pointer-events manipulation from modal scroll locking (overflow-only) to eliminate CSS-based click blocking. Console logging gated behind debug flags (debugReloadOnResume, debugDOMBridge) for production cleanliness. Reload-on-resume workaround (flutterflow-reload.ts) disabled since root cause is fixed - this preserves audio playback when users minimize mid-listen.
