...
Button interaction fix (Nov 2025): Fixed critical button freeze bug in FlutterFlow web view environment. Root cause: FlutterFlow's WebView intermittently detaches React's root event delegation listener during background/resume cycles, causing onClick handlers to stop firing while CSS :active still works. Solution: Implemented comprehensive DOM Event Bridge system with global coverage. Created dom-event-bridge.ts with `useDomBridgeAction` hook that registers resilient capture-phase document listeners bypassing React entirely. Integrated bridge into shared Button component (@/components/ui/button.tsx) so ALL buttons throughout the app automatically get dual-layer click handling: React's onClick (for normal browsers) AND DOM-level fallback (for when FlutterFlow breaks React). Bridge re-attaches itself every 5 seconds and includes health checks on page resume to detect React delegation failures. Additionally removed pointer-events manipulation from modal scroll locking (overflow-only) to eliminate CSS-based click blocking. This architecture ensures all user interactions remain functional after app minimize/resume in FlutterFlow.
