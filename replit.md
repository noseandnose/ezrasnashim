...
Button interaction fix (Nov 2025): Fixed critical button freeze bug in FlutterFlow web view environment. Root cause: React's synthetic event system uses event delegation (click/touch listeners on document root). When FlutterFlow suspends the page during backgrounding, React lifecycle hooks (useLayoutEffect) never run, leaving fullscreen modal's `pointer-events: none` stuck on body/scroll containers. This blocks React's event delegation while CSS `:active` pseudo-class still works (explaining why visual feedback appears but clicks don't fire). Solution: Added persistent global resume handler at module level in fullscreen-modal.tsx that stores itself on `window` to survive FlutterFlow suspensions. Handler attaches to multiple resume signals (visibilitychange, webkitvisibilitychange, focus, pageshow) and re-attaches every 10 seconds as FlutterFlow detaches event listeners. On page resume, synchronously calls `forceResetScrollLock()` if no modals are active, immediately restoring pointer-events and allowing React's event system to function.
