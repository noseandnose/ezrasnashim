...
- **PWA Audio Playback & Version Check Fix**: Fixed two critical PWA issues: (1) Audio player not working in installed PWA mode (stayed at 0:00, play button unresponsive) - service worker was intercepting audio streaming requests and returning cached HTML on transient failures. (2) Version check error "SyntaxError: Unexpected token '<'" - /api/version endpoint was being cached by service worker and serving stale HTML. Solution: Updated service worker to bypass caching entirely for audio/video requests (destination checks, media extensions, range headers, /api/media-proxy/ paths) and /api/version endpoint. Removed /api/version from CRITICAL_API_PATTERNS. Updated cache version to v1.0.1-1761803468677 to force cache refresh. Updated main.tsx version check to use absolute URL with timestamp query parameter, explicit cache headers, and content-type validation to completely bypass old service workers. Audio now streams directly to network without interference, version checks always fetch fresh JSON even with stale service workers.
- **Table Inspirations Navigation Arrow Fixes**: Fixed two issues with media navigation arrows in Creative Jewish Living (Table Inspirations) modal: (1) Arrows jumping position when clicking between media items - changed from percentage-based (`top: 50%`) to fixed positioning (`top: 160px`) to maintain consistent vertical placement regardless of content height. (2) Arrows not working reliably in fullscreen mode - touch event handlers on parent container were interfering with button clicks. Added `stopPropagation()` to all button events (onClick, onTouchStart, onTouchEnd), added `z-10` class for proper stacking, and added `preventDefault()` on touch end to prevent default touch behaviors. Arrows now work 100% reliably in both regular modal and fullscreen views.
