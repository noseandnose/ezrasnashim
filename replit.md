...
Button interaction fix (Nov 2025): Fixed critical button freeze bug when app is minimized/backgrounded. Root cause: fullscreen modal system uses `requestAnimationFrame` for scroll-lock cleanup, which doesn't execute when app is backgrounded, leaving `pointer-events: none` stuck on scroll container. Solution: exported `forceResetScrollLock()` from fullscreen-modal.tsx and enhanced visibility change handler in App.tsx to detect open modals (checking both `[role="dialog"][data-state="open"]` for Radix dialogs and `[data-fullscreen-modal]` for custom fullscreen modals) before conditionally calling force reset. When no modals are open on resume, handler forcibly restores pointer events on scroll container and all interactive elements with 100ms delay and reflow trigger. Prevents frozen UI after minimize/resume cycles while preserving modal isolation.
