...
Button interaction fix (Nov 2025): Fixed critical button freeze bug when app is minimized/backgrounded in PWA/app environments. Root cause: fullscreen modal system uses `requestAnimationFrame` for scroll-lock cleanup, which doesn't execute when app is backgrounded, leaving `pointer-events: none` stuck on scroll container. Solution: (1) Modified App.tsx visibility restoration to ALWAYS force reset scroll locks first before checking modal state, then conditionally reapply locks for genuinely open modals. (2) Updated fullscreen modal cleanup to use immediate synchronous restoration when `document.visibilityState !== 'visible'` or in web views, while preserving double-requestAnimationFrame for regular browsers (gesture protection). (3) Created environment detection utility (`client/src/utils/environment.ts`) for web view identification. This ensures pointer-events are properly restored even when JavaScript event loop is suspended during backgrounding.
