...
Button interaction fix (Nov 2025): Fixed critical button freeze bug in FlutterFlow web view environment. Root cause: FlutterFlow's WebView swallows touchend/click events entirely after certain background/resume cycles, making all buttons unresponsive (CSS :active still works since pointerdown fires, but the release event never reaches JavaScript). Solution: Implemented reload-on-resume system (flutterflow-reload.ts) that detects webview environments and automatically reloads the page when the app regains visibility after being backgrounded. Uses visibilitychange and pageshow events with debouncing. System includes comprehensive logging to debug webview detection, manual override via localStorage.setItem("forceReloadOnResume", "true"), and detection of Flutter global objects. Also fixed modal click-through bug where clicking X button would trigger buttons beneath the modal - all onClose calls now deferred with requestAnimationFrame to prevent event retargeting during DOM unmounting. Additionally removed pointer-events manipulation from modal scroll locking (overflow-only) to eliminate CSS-based click blocking. This ensures reliable button functionality in all environments.
