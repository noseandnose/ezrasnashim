...
Button interaction fix (Dec 2025 UPDATED): Completely redesigned the FlutterFlow WebView button freeze fix to avoid disruptive page reloads. The original issue: After backgrounding and resuming the FlutterFlow app, buttons would show CSS :active state (pointerdown fires) but JavaScript click/touchend handlers wouldn't execute (events swallowed by WebView). Original solution was force page reload on resume - this worked but disrupted users mid-prayer or during audio shiurim.

**NEW SOLUTION (Dec 2025)**: Uses pointerup-based tap detection instead of relying on click/touchend events. Since pointerdown fires (causing CSS :active), pointerup should also fire - they're part of the same Pointer Events API. The DOM event bridge (dom-event-bridge.ts) now:
1. Tracks pointerdown targets and timestamps
2. Listens for pointerup events at document level with capture phase
3. Validates taps (same element, < 500ms between down/up)
4. Invokes registered actions via data-action attributes when touchend/click are swallowed
5. Immediately re-attaches all listeners when page becomes visible (no 5-second wait)
6. Uses both pointerup AND click/touchend for maximum compatibility

Force reload is now DISABLED by default (opt-in via localStorage.setItem("forceReloadOnResume", "true")). Critical interactive elements (bottom navigation, header buttons, menu items, modal close buttons) have data-action attributes that register with the DOM bridge for resilient handling.

Debug mode: localStorage.setItem("debugDOMBridge", "true") for DOM bridge logging, localStorage.setItem("debugReloadOnResume", "true") for reload system logging.
Modal freeze fix (Dec 2025): Fixed app freezing when clicking multiple buttons rapidly during background/foreground transitions or rapid foreground navigation. Root causes: (1) requestAnimationFrame callbacks never fire when page is hidden/backgrounded, (2) safeClose function recreated on every render causing constant DOM bridge re-registration, (3) no double-close protection for rapid clicking. Solution: Created safeClose helper function that closes modals immediately (without rAF) when document.visibilityState !== 'visible', otherwise uses rAF with a 250ms timeout fallback. Added isClosingRef flag to prevent double-close from rapid clicking. Wrapped safeClose in useCallback with onClose dependency for stable function reference. Added global visibility change safety net that detects stuck modals (counter > 0 but no modal elements exist) and force resets scroll lock when page becomes visible. This prevents frozen UI states during rapid button taps in both foreground and background scenarios.
