...
Button interaction fix (Nov 2025): Fixed critical button freeze bug when app is minimized/backgrounded in PWA/app environments. Root cause: fullscreen modal system uses `requestAnimationFrame` for scroll-lock cleanup, which doesn't execute when app is backgrounded, leaving `pointer-events: none` stuck on scroll container. Additionally, `visibilitychange` events are unreliable in PWA/app wrappers, especially on quick minimize/resume cycles. Solution: Created `restoreInteractions()` helper in App.tsx that listens to multiple resume events (visibilitychange, focus, pageshow) to ensure restoration works across all PWA lifecycle scenarios. Helper detects open modals (checking both `[role="dialog"][data-state="open"]` for Radix dialogs and `[data-fullscreen-modal]` for custom fullscreen modals) before conditionally calling `forceResetScrollLock()`. When no modals are open on resume, forcibly restores pointer events on scroll container and all interactive elements with 100ms delay and reflow trigger. Multi-event approach ensures at least one resume signal fires even when individual events fail. Prevents frozen UI after minimize/resume cycles while preserving modal isolation.
