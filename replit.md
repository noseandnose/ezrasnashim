...
- **PWA iOS Safe Area Fix**: Resolved critical issue where header covered content in iOS PWA mode. Header uses `position: fixed; top: 0` with `padding-top: calc(env(safe-area-inset-top) + 0.625rem)` to position below iPhone notch. Content padding now dynamically reads header's computed padding-top (which includes resolved safe-area-inset) and calculates total header height by parsing computed styles. ResizeObserver tracks header size changes and recalculates content padding when safe-area resolves asynchronously on iOS PWA. This ensures "Good Afternoon" section and all top content is fully visible without being hidden behind header across all device modes (PWA, browser, webview).
