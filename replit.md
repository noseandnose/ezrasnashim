...
Button interaction fix (Nov 2025): Fixed critical button freeze bug in FlutterFlow mobile app. Root cause identified: DOM Event Bridge (dom-event-bridge.ts) had a setInterval that periodically removed and re-added event listeners every 5 seconds. When app went to background, JavaScript timers paused; on resume, the timer would fire immediately and REMOVE listeners, causing touchend/click events to be completely lost until the next cycle. Solution: Removed the problematic setInterval - event listeners now attach once and stay attached. DOM bridge still provides resilience via capture-phase document listeners with data-action attributes, bypassing React's event delegation when needed. Also fixed modal click-through bug where clicking X button would trigger buttons beneath the modal - all onClose calls now deferred with requestAnimationFrame to prevent event retargeting during DOM unmounting. Additionally removed pointer-events manipulation from modal scroll locking (overflow-only) to eliminate CSS-based click blocking. Console logging gated behind debug flags (debugReloadOnResume, debugDOMBridge) for production cleanliness. Reload-on-resume workaround (flutterflow-reload.ts) disabled since root cause is fixed - this preserves audio playback when users minimize mid-listen.
