...
export function registerPrayerRoutes(app: Express, deps: PrayerRouteDeps) {
  ...
  app.get("/api/mincha/prayer", async (_req: Request, res: Response) => {
    try {
      const prayers = await storage.getMinchaPrayers();
      return res.json(prayers);
    } catch (error) {
  ...
  app.get("/api/maariv/prayer", async (_req: Request, res: Response) => {
    try {
      const prayers = await storage.getMaarivPrayers();
      return res.json(prayers);
    } catch (error) {
  ...
  // Brochas routes
  app.get("/api/brochas", async (_req: Request, res: Response) => {
    try {
      const brochas = await storage.getBrochas();
      return res.json(brochas);
    } catch (error) {
  ...
  app.get("/api/brochas/daily", async (_req: Request, res: Response) => {
    try {
  ...
  app.get("/api/brochas/special", async (_req: Request, res: Response) => {
    try {
